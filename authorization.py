...
@user_router.post("/login/bot/confirm")
async def confirm_link(
    ...
    select_stmt = select(TGLinks).where(TGLinks.token == data.token, TGLinks.expires_at > current_time)
    result = await session.execute(select_stmt)
    ...
    update_link = update(TGLinks).where(TGLinks.token == data.token).values(used=True)
    update_user = update(User).where(User.user_id == link.user_id).values(telegram_id=data.telegram_id)
...
@user_router.post("/login/bot/check{telegram_user_id}")
async def check_tg_user(
    ...
    stmt = select(User.telegram_id).where(User.telegram_id == telegram_user_id)
...
@user_router.delete("/login/bot/{telegram_user_id}")
async def delete_user(
    ...
    del_stmt = update(User).where(User.telegram_id == telegram_user_id).values(telegram_id=None)
    await session.execute(del_stmt)
    await session.commit()

@user_router.get("/login/bot/all")
async def all_users(
        session:Annotated[AsyncSession, Depends(get_db)],
        x_api_key:Annotated[str|None, Header()] = None
):
    ...
    select_stmt = select(User.telegram_id)
    result = await session.execute(select_stmt)
    rows = result.scalars()
